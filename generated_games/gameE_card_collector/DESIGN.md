...
> 状态：**全部已实现并校准 ✅** —— 成本曲线、转生闭环（新卡池 + 黄金宝箱）、等级上限随转生成长、死抽问题修复。
> 当前参数：`COST_SCALE=100`、`PER_POINT=200000`、`MAX_LEVEL_BASE=5` + 每转生 +1、`LEVEL_GROWTH_RATE=1.5`。
> 本文档记录「提前通关」问题的诊断与解法演进（§7-§10 为校准实测）。
...
参数：`COST_SCALE = 1000`（当时值，兜底墙 ≈ 2h47m）。

> **⚠️ 上述结论已在 §10 被修正。** 「`COST_SCALE` 不是节奏调节器」只在 1000 这个取值下成立——
> 那时一轮有约 5000 箱，成本曲线来不及爬升。改为 100 后，成本墙在 **18m54s** 到达、真正在一轮内生效，
> 且一轮箱数降到约 670，反而成为解决死抽与成长期问题的关键杠杆。见 §10.3。
...
| 参数 | 取值 | 说明 |
|---|---|---|
| `COST_SCALE` | **100**（已应用） | 每 100 累计收入成本 +1；**决定一轮包含多少箱**，见 §10 |
| `PER_POINT` | **200000**（已应用） | 转生时每 N 累计收入换 1 P；在一轮箱数之上决定时长 |
...
用 `tools/balance-sim.mjs` 在 6 个种子下模拟，结论高度稳定。

> **本节结论已被 §10 修正。** §7 当时认为「节奏由 `PER_POINT` 决定，`COST_SCALE` 只是兜底」，
> 那是**在 `COST_SCALE = 1000` 的前提下**成立的：一轮长达 5000 箱，成本曲线根本来不及爬升。
> §10 发现 `COST_SCALE` 才是真正的总闸门——它决定**一轮包含多少箱**，而那是内容深度的分母。
> 现值已改为 100，本节表格保留原口径以记录演进。
...
- **满级后的重复卡几乎无价值，且随等级上限膨胀而加速贬值**：`MAXED_DUPLICATE_COIN_MULTIPLIER = 3` 的返还额恒为 `baseIncome × 3`，与等级无关；而满级卡收入按 `1.5^(L-1)` 指数增长。因此返还的**相对价值随上限抬升而指数衰减**：
  ...
  §7.5 早期版本把该比值记为「固定的 0.593 秒」——那只在 `MAX_LEVEL = 5` 时成立。
  §9 引入**上限随转生持续增长**后，问题被放大：转生越多次，满级卡的重复越接近零收益，
  后期轮次的抽卡逐步退化为空转。这需要设计决策（见上），不是调参数能解决的。

  **实测「死抽」占比**（一轮内抽到已满级卡的比例，按当前策略模拟 6 轮）：

  | 轮次 | 上限 | 抽卡数 | 死抽数 | 占比 | 返还金币 |
  |---|---|---|---|---|---|
  | 1 | Lv5 | 5281 | 5186 | **98.2%** | 35982 |
  | 2 | Lv6 | 5172 | 5004 | 96.8% | 35865 |
  | 3 | Lv7 | 5061 | 4830 | 95.4% | 33726 |
  | 4 | Lv8 | 4899 | 4549 | 92.9% | 30849 |
  | 5 | Lv9 | 4708 | 4174 | 88.7% | 27117 |
  | 6 | Lv10 | 4594 | 3840 | 83.6% | 23988 |
  | **合计** | | 29715 | 27583 | **92.8%** | |

  即**一轮里 9 成以上的开箱不产生任何升级**，只剩返还金币。根因是**抽卡数与升级槽位严重不匹配**：
  一轮约 5000 次抽卡，而 19 张卡按几何阈值练满只需约 300 次有效升级。
  更关键的是**高权重的低稀有度卡先满级**（common 权重 50），满级后继续吃掉约一半的抽卡。

  返还金币并非为零——首轮约占总收入的 **14.4%**（35982 / 250000）——但单次体验是「开了个寂寞」。
- **重复卡稀释进度**：P20 后卡池 25 张、阈值随上限增长，抽到特定卡练满的期望抽数很高，后期成长感可能偏慢。
...
## 10. 死抽问题：满级卡吃掉了绝大多数开箱（已修复 ✅）

§7.5 记录了「满级重复卡返还随等级贬值」。量化后发现问题严重得多：
**一轮里 9 成以上的开箱不产生任何升级**。

### 10.1 先证伪两个直觉方案

实测（`tools/balance-sim.mjs`，默认策略）：
...
- **「满级卡退出掉落池」无效**：它靠把抽卡转移到还能升级的卡上，但集合在约 500 次抽卡后
  **全部**满级，闲置池仅剩约 100 次升级事件，无对象可转移。它反而让集合满得更快。
- **「提高返还倍数」有硬性上限**：单箱期望基础收入为 2.34，箱价 10，
  故 `M ≥ 4.3` 时**买箱自我盈利**，产生无限刷钱循环（模拟器的 `buy loop did not terminate`
  守卫正是捕获了这个）。返还最多从 3 提到约 4.2，仅 1.4 倍，远不足以覆盖 98% 的死抽。

> 注：模拟器假设「有钱就瞬间全部买开」，每轮约 5000 次抽卡（≈2.5 次/秒）——真人手动按键达不到。
> 但按点击速率 0.5 次/秒重测，死抽率仍有 **94.7%**，所以该问题与手速无关。

### 10.2 根因：一轮的箱数远超集合的升级槽位

| 等级上限 | 可用升级事件 | 覆盖一轮抽卡的比例 |
|---|---|---|
| Lv6（首轮） | 133 | **2.7%** |
| Lv10 | 703 | 14.1% |
| Lv14 | 3686 | 73.7% |

19 张卡在 Lv6 只有 **133 次**有效升级，而一轮有约 5000 次抽卡——**这才是 98% 的出处**。
它是「一轮太长、内容太浅」的错配，不是返还数值问题。

### 10.3 真正有效的杠杆：`COST_SCALE`（一轮包含多少箱）

成本涨得越快，一轮开的箱越少，梯子相对越耐用：

| `COST_SCALE` | 抽卡数 | 死抽率 | 成长期 | 收入天花板 |
|---|---|---|---|---|
| 1000（原值） | 5281 | 98.2% | 0m36s | 381 |
| 250 | 1555 | 93.9% | 1m22s | 381 |
| **100（已应用）** | **665** | **85.7%** | **23m25s** | **381** |
| 50 | 368 | 76.1% | 47m31s | 278（饿死内容） |

`COST_SCALE = 100` 一次性改善三项且不牺牲任何已验证指标：

1. **死抽率 98.2% → 85.7%**，且随轮次继续下降（第 5 轮降至 **50.9%**）。
2. **成长期 36 秒 → 23 分 25 秒**（占一轮 ~80%）——这正是 §8 遗留的问题。
3. **成本墙首次在一轮内生效**（18m54s 到达），真正兑现 §3 的原始设计意图；
   原值 1000 下它要 2h47m 才触发，等于从未生效。

`COST_SCALE = 50` 已越过下限（收入天花板从 381 掉到 278，内容被饿死）。

### 10.4 联动重校 `PER_POINT`

成本曲线变了，转生阈值需同步下调（6 种子实测）：

| `PER_POINT` | 首次转生 P3 |
|---|---|
| 150000 | 22m40s |
| **200000（已应用）** | **26m55s ~ 29m15s** |
| 250000 | 35m49s |

### 10.5 应用后的多轮轨迹

| 轮次 | 时长 | 成长期 | 上限 | 抽卡 | 死抽率 | 收入天花板 |
|---|---|---|---|---|---|---|
| 1 | 29m15s | 23m25s | 6 | 665 | 85.7% | 381/s |
| 2 | 12m59s | 8m34s | 7 | 609 | 75.4% | 552/s |
| 3 | 22m02s | 18m00s | 8 | 691 | 70.0% | 849/s |
| 4 | 25m14s | 11m32s | 9 | 752 | 57.3% | 1493/s |
| 5 | 2h30m | 2h00m | 10 | 950 | 50.9% | 1747/s |

死抽率逐轮下降，因为等级上限成长让梯子变深——§9 与 §10 是相互增强的。

### 10.6 剩余问题

`COST_SCALE = 100` 把死抽率降到 85.7%，**但没有消除它**。剩余部分来自「一轮仍略长于内容」：
首轮梯子只覆盖一轮抽卡的 ~14%。要完全填满需 Lv15（约 10 次转生）。
后续可选手段：提高 `LEVEL_GROWTH_RATE`（1.5 → 1.9 可让梯子在 5 次转生内填满），
代价是升级收益稀释、头部进度变差。属调优空间，非缺陷。
