...
> 见 [`../server/`](../server/README.md)）；**阶段 2（链上领取）未实施**。
...
| 内容 | 说明 |
|---|---|
| 部署 ERC-1155 | Polygon PoS |
| `POST /claim` | 后端签 EIP-712 凭证 |
| 前端钱包连接 | wagmi/viem，玩家自付 gas 领取 |
| 领取状态同步 | `chain_claims` 表标记已上链 |

**成本**：合约部署约 $0.02–0.5，之后每玩家领取由玩家自付。
