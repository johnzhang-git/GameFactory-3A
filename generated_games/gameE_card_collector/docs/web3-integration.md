...
>
> 状态：**方案设计，未实施**。前置依赖：游戏目前**没有任何存档**（见 §1.1）。
...
### 阶段 1：存档与钱包账号（**最小可行，先做这个**）

| 内容 | 说明 |
|---|---|
| 后端骨架 | 单 Node 服务 + SQLite |
| SIWE 登录 | 钱包签名登录，地址即账号 |
| 存档读写 | 存档 JSON 存库，前端启动时拉取 |
| 抽卡移到后端 | 复用 `economy.js`，改用服务端随机种子 |
| **链上交互** | **无** |
...
| 现有文件 | 阶段 1 改动 | 阶段 2 改动 |
|---|---|---|
| `catalog.js` | **不改**（后端直接复用） | 新增 tokenId ↔ 卡名映射 |
| `economy.js` | **不改**（后端复用规则） | 新增「已领取」标记 |
| `game.js` | 抽卡改为调用后端 API | 新增 `claimToWallet()` |
| `index.js` | 新增登录/存档 UI | 新增钱包连接与领取按钮 |
| `renderer.js` | **不改** | **不改** |
| 新增 `server/` | 全部 | 增加 `/claim` 与签名模块 |
